...
exp2_005_df <- get_genes_2exp(0.05)   # 70 genes
exp2_01_df  <- get_genes_2exp(0.1)    # 33 genes
exp2_025_df <- get_genes_2exp(0.25)   # 4 genes
...
cor(meta_gsea_input$log2FC_Day0,meta_gsea_input$log2FC_Unmanipulated,method = "spearman") 
...
Day0_GSEA <- '.../GSEA/GSEA_Day0_all_genes_Enriched_Pathways.xlsx'
Day0_GSEA_results <- read_excel(Day0_GSEA, sheet = 1)
Day0_GSEA_results # 8800 rows 

Day0_Unmani_shared_genes_GSEA <- '.../GSEA/GSEA_Day0_Unmani_shared_genes_Enriched_Pathways.xlsx'
...
pathway_labels <- c(
  ...
  "FISCHER_DREAM_TARGETS" = "DREAM cell-cycle",
  "CROONQUIST_IL6_DEPRIVATION_DN" = "IL6 signaling",
  "CROONQUIST_NRAS_SIGNALING_DN" = "NRAS signaling",
  "REACTOME_SIGNALING_BY_RHO_GTPASES_MIRO_GTPASES_AND_RHOBTB3" = "Rho GTPase signaling",
  "PID_CMYB_PATHWAY" = "c-MYB reg. program",
  "HALLMARK_INFLAMMATORY_RESPONSE" = "Inflammatory response",
  "MARTENS_TRETINOIN_RESPONSE_DN" = "ATRA response to APL",
  "GRAHAM_CML_QUIESCENT_VS_NORMAL_QUIESCENT_UP" = "Quiescent LSC program"
)
...
pathway_labels_shared <- c(
  ...
  "KIM_WT1_TARGETS_DN" = "WT1 regulatory program",
  "BYSTRYKH_HEMATOPOIESIS_STEM_CELL_QTL_TRANS" = "HSC trans-QTL",
  "HALLMARK_APOPTOSIS" = "Apoptotic signaling program",
  "REACTOME_CYTOKINE_SIGNALING_IN_IMMUNE_SYSTEM" = "Cytokine signaling pathway"
)
...
Day0_Unmani_shared_GSEA_dotplot <- ggplot(
  ...
  labs(
    x = "NES",
    y = NULL,
    title = "Day0 Unmanipulated Shared\nDVGs GSEA"
  ) +
  scale_x_continuous(
    breaks = c(1.4, 1.8),
    limits = c(1.3, 1.95),
  ...
  theme(
    ...
    axis.text.x  = element_text(size = 18, color = "black"),
    axis.text.y  = element_text(size = 20, color = "black"),
    plot.title = element_text(hjust = 0.5, size = 20, face = "bold",lineheight = 1.1),
    legend.title = element_text(color = "black"),
    legend.text  = element_text(color = "black"),
    plot.margin = margin(5, 25, 5, 5)
  ) +
