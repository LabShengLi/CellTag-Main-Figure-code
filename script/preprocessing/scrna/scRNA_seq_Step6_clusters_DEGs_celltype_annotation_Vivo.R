...
RDSname <- "cross.exp.combined.vivo.hto.celltag.rds"

outdir <- paste0(wdir, "biology"); dir.create(outdir, showWarnings = FALSE)
...
write.table(x = markers, paste(outdir, "/markers_vivo.tsv", sep = ""), sep = "\t", quote = F, row.names = F, col.names = T)
markers <- read.table(paste0(outdir, "/markers_vivo.tsv"), header = T)

library("dplyr")
top <- markers %>%
	group_by(cluster) %>%
	top_n(n = 10, wt = avg_log2FC); top <- top[!duplicated(top$gene),]; dim(top);
write.csv(top, file = paste0(wdir, "top10_vivo.csv"))
...
graphics.off()
pdf(file = paste0(wdir, "top10_vivo.pdf"), height = 14)
...
graphics.off()
pdf(file = paste0(wdir, "celltypeMarkersHeatVivo.pdf"), width = 10, height = 22)
...
save(normalized_markers_meanexp, file = paste0(wdir, "heatmap_input.rda"))
